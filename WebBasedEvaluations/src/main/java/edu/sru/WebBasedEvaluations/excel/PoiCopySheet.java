...
//Class used to copy a sheet from one excel workbook into another excel workbook.
//This would keep formatting, data, and all attributes of the sheet, unlike my implementation in progress.
//reference: https://robsnotebook.com/copy-a-sheet-to-a-different-workbook-in-apache-poi/, October 2023
public class PoiCopySheet {
    private Set<CellRangeAddress> mergedRegions_ = new TreeSet<CellRangeAddress>();
        ...
        for (; j <= srcRow.getLastCellNum(); j++)
        {
            ...
            if (srcCell != null)
            {
                ...
                if (mergedRegion != null)
                {
                    ...
                    //CellRangeAddress wrapper = new CellRangeAddress(dstMergedRegion);
...
//    }
    ...
    private CellStyle getSameCellStyle(CellStyle srcCellStyle) {
        ...
        for (int dstStyleIdx = 0; dstStyleIdx < totalSrcCellStyles; ++dstStyleIdx)
        {
            ...
            if (cellStylesEquals(currentCellStyle, dstWorkbook_, srcCellStyle, srcWorkbook_, expectedSrcDataFormat))
            {
    ...
    private static boolean cellStylesEquals(CellStyle leftStyle, Workbook leftWorkbook, CellStyle rightStyle,
                                            Workbook rightWorkbook, short rightDataFormat) {
